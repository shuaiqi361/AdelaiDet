...
class DTMRInstOutputs(object):
    def __init__(
        ...
        self.loss_on_mask = cfg.MODEL.DTMRInst.LOSS_ON_MASK
        self.loss_on_code = cfg.MODEL.DTMRInst.LOSS_ON_CODE
        self.mask_loss_type = cfg.MODEL.DTMRInst.MASK_LOSS_TYPE
        self.num_codes = cfg.MODEL.DTMRInst.NUM_CODE
        self.mask_size = cfg.MODEL.DTMRInst.MASK_SIZE
        self.mask_sparse_weight = cfg.MODEL.DTMRInst.MASK_SPARSE_WEIGHT
        self.mask_loss_weight = cfg.MODEL.DTMRInst.MASK_LOSS_WEIGHT
        self.sparsity_loss_type = cfg.MODEL.DTMRInst.SPARSITY_LOSS_TYPE

        # Matcher to assign box proposals to gt boxes
        self.proposal_matcher = Matcher(
            cfg.MODEL.DTMRInst.IOU_THRESHOLDS,
            cfg.MODEL.DTMRInst.IOU_LABELS,
    ...
    def DTMRInst_losses(
        ...
        code_targets, dtm_targets = self.mask_encoding.encoder(mask_targets)
        ...
        if self.loss_on_mask:
            ...
            if 'mask_dice' in self.mask_loss_type:
                ...
                union_ = torch.sum(mask_pred_ ** 2, 1) + torch.sum(mask_targets ** 2, 1)
                _loss = (1. - overlap_ / (union_ + 1e-5)) * ctrness_targets * self.mask_size ** 2
                dice_loss = _loss.sum() / max(ctrness_norm * self.mask_size ** 2, 1.0)
        ...
        if self.loss_on_code:
            ...
            if self.mask_sparse_weight > 0.:
                if self.sparsity_loss_type == 'L1':
                    ...
                    mask_loss = total_mask_loss * self.mask_loss_weight + \
                    ...
                    mask_loss = total_mask_loss * self.mask_loss_weight + sparsity_loss * self.mask_sparse_weight
                    ...
                    sparsity_loss = sparsity_loss.sum() / max(ctrness_norm * self.num_codes, 1.0)
                    total_mask_loss = total_mask_loss * self.mask_loss_weight + \
                                sparsity_loss * self.mask_sparse_weight
                else:
    ...
    def losses(self):
        """
        Return the losses from a set of DTMRInst predictions and their associated ground-truth.
    ...
    def predict_proposals(self):
        ...
        for i in range(num_images):
            ...
            per_image_masks = torch.clamp(per_image_masks, min=0.01, max=0.99)
    ...
    def forward_for_single_feature_map(
        ...
        pre_nms_top_n = candidate_inds.reshape(N, -1).sum(1)  # this is suggested when running on my machine
